...
<asp:Content ID="Content2" ContentPlaceHolderID="bodyPlaceHolder" runat="server">
        ...
        <ContentTemplate>
            ...
            <section class="content">
                <div class="box box-primary">
                    ...
                    <asp:SqlDataSource ID="dsClaimManual" runat="server" ConnectionString="<%$ ConnectionStrings:ClaimAppConnectionStringLive %>" SelectCommand="select * from ClaimMotorVehicle Where ([Pid] = @Pid)">
                        <SelectParameters>
                            <asp:QueryStringParameter DefaultValue="0" Name="Pid" QueryStringField="Pid" />
                        </SelectParameters>
                    </asp:SqlDataSource>
                    <asp:SqlDataSource ID="dsWorkshop" runat="server" ConnectionString="<%$ ConnectionStrings:ClaimAppConnectionStringLive %>" SelectCommand="Select * from Master_Bengkel Where Status = 'True'"></asp:SqlDataSource>
                    <asp:SqlDataSource ID="dsCbPolicyNo" runat="server" ConnectionString="<%$ ConnectionStrings:ClaimAppConnectionCoreLive %>" SelectCommand="  select PolNo from PolMain Where PolTo &gt; convert(date,getdate(),103) AND ClassCode = '3' AND Status in('v','P') and ClientCode = @ClientCode">
                        <SelectParameters>
                            <asp:ControlParameter ControlID="ctl00$bodyPlaceHolder$BootstrapFormLayout1$cbClient" Name="ClientCode" PropertyName="Value" />
                        </SelectParameters>
                    </asp:SqlDataSource>
                    <asp:SqlDataSource ID="dsNoPoliceAndVehicle" runat="server" ConnectionString="<%$ ConnectionStrings:ClaimAppConnectionCoreLive %>" SelectCommand=" SELECT DISTINCT B.PoliceNo , B.Type AS VehicleModel
 ...
 WHERE A.PolNo = @PolNo AND B.ChassisNo = @ChasisNo AND B.EngineNo = @EngineNo">
                    ...
                    <asp:SqlDataSource ID="dsCbChasisNo" runat="server" ConnectionString="<%$ ConnectionStrings:ClaimAppConnectionCoreLive %>" SelectCommand="SELECT DISTINCT B.ChassisNo AS CHASSIS_NO
 FROM POLMain A
 ...
 WHERE A.PolNo = @PolNo">
                    ...
                    <asp:SqlDataSource ID="dsCbEngineNo" runat="server" ConnectionString="<%$ ConnectionStrings:ClaimAppConnectionCoreLive %>" SelectCommand="SELECT DISTINCT B.EngineNo AS ENGINE_NO
 FROM POLMain A
 LEFT OUTER JOIN POLDetail_3 B ON A.PolID = B.PolID
 WHERE A.PolNo = @PolNo  AND B.ChassisNo = @ChasisNo ">
                    ...
                    <asp:SqlDataSource ID="dsClientName" runat="server" ConnectionString="<%$ ConnectionStrings:ClaimAppConnectionCoreLive %>" SelectCommand="   Select DISTINCT b.ClientCode,B.ClientName,C.DivName,a.PoliceNo from POLDetail_3 A
  ...
  Where PoliceNo = @PoliceNo  AND  B.PolTo &gt; convert(date,getdate(),103) AND B.ClassCode = '3' AND B.Status in('v','P')">
                    ...
                    <asp:SqlDataSource ID="dsCbInsurance" runat="server" ConnectionString="<%$ ConnectionStrings:ClaimAppConnectionCoreLive %>" SelectCommand=" SELECT DISTINCT C.ClientType AS CLIENT_TYPE,C.Type,C.Name AS INSURANCE,UPPER(C.Type + C.Name) AS INSURANCENAME
 FROM indoins_back.dbo.POLMain A
 LEFT OUTER JOIN indoins_back.dbo.POLDetail_3 B ON A.PolID = B.PolID
 ...
 WHERE A.PolNo = @PolNo AND B.ChassisNo = @ChasisNo AND B.EngineNo = @EngineNo AND C.ClientType = 'U'">
                    ...
                    <asp:SqlDataSource ID="dsDivName" runat="server" ConnectionString="<%$ ConnectionStrings:ClaimAppConnectionCoreLive %>" SelectCommand="select DISTINCT a.ClientCode,a.DivCode,b.DivName
from Polmain a 
